...
#### Grid
fco2_xgb_grid <- expand.grid(
  loss_reduction = c(0.01, 0.05, 1, 2, 4)
)
...
fco2_xgb_last_fit <- fco2_xgb_wf  |>  
  last_fit(
    split = fco2_initial_split,
    control = control_grid(save_pred = TRUE, verbose = FALSE, allow_par = TRUE),
    metrics = metric_set(rmse)
  )
```
...
```{r}
fco2_xgb_best_params <- select_best(fco2_xgb_model, "rmse")
fco2_xgb_wf <- fco2_xgb_wf %>% finalize_workflow(fco2_xgb_best_params)
